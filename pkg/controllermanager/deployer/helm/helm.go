...
func (deployer *Deployer) handleDescription(desc *appsapi.Description) error {
	...
	if desc.DeletionTimestamp != nil {
		err := utils.ProtectAutoGeneratedResources(deployer.clusternetClient,
			klog.KRef(desc.Namespace, desc.Labels[known.ClusterNameLabel]),
			klog.KRef(desc.Labels[known.ConfigSubscriptionNamespaceLabel], desc.Labels[known.ConfigSubscriptionNameLabel]))
		if err != nil {
			return err
		}
...
func (deployer *Deployer) handleHelmRelease(hr *appsapi.HelmRelease) error {
	...
	if hr.DeletionTimestamp != nil {
		err := utils.ProtectAutoGeneratedResources(deployer.clusternetClient,
			klog.KRef(hr.Namespace, hr.Labels[known.ClusterNameLabel]),
			klog.KRef(hr.Labels[known.ConfigSubscriptionNamespaceLabel], hr.Labels[known.ConfigSubscriptionNameLabel]))
		if err != nil {
			return err
		}
