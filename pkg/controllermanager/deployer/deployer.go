...
func (deployer *Deployer) handleBase(base *appsapi.Base) error {
	...
	if base.DeletionTimestamp != nil {
		err := utils.ProtectAutoGeneratedResources(deployer.clusternetClient,
			klog.KRef(base.Namespace, base.Labels[known.ClusterNameLabel]),
			klog.KRef(base.Labels[known.ConfigSubscriptionNamespaceLabel], base.Labels[known.ConfigSubscriptionNameLabel]))
		if err != nil {
			return err
		}
