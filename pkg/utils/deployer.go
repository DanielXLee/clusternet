...
// ProtectAutoGeneratedResources protects auto-generated resources from being deleted by users.
func ProtectAutoGeneratedResources(clusternetClient clusternetclientset.Interface, mclsRef, subRef klog.ObjectRef) error {
	sub, err := clusternetClient.AppsV1alpha1().Subscriptions(subRef.Namespace).Get(context.TODO(), subRef.Name, metav1.GetOptions{})
	if err != nil && !apierrors.IsNotFound(err) {
		return err
	}
	// Block deletion if the Subscription is not marked for deletion and BindingClusters contains the cluster the resource belongs to.
	if sub != nil && sub.DeletionTimestamp == nil && ContainsString(sub.Status.BindingClusters, mclsRef.String()) {
		msg := fmt.Sprintf("block deleting current resource until subscription %s gets deleted or unbound from subscription", subRef.String())
